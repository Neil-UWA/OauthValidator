...
describe('OauthValidator', function() {
  ...
  describe('#constructor', ()=>{
    it('works', ()=>{
      let oauthValidator = new OauthValidator('qq');
  ...
  describe('#validate', ()=>{
    ...
    context('login with weibo', ()=>{
      beforeEach(()=>{
        postStub =
          sinon.stub(request, 'postAsync')
                .returns(Promise.resolve([{}, {uid: 123}]));
      });

      afterEach(()=> postStub.restore());

      it('should call request.postAsync once', ()=>{
        let oauthValidator = new OauthValidator('weibo');
    ...
    context('login with wechat', ()=>{
      beforeEach(()=>{
        getStub =
          sinon.stub(request, 'getAsync')
               .returns(Promise.resolve([{}, {openid: 'xxxx'}]));
      });

      afterEach(()=> getStub.restore());

      it('should call request.getAsync once', ()=>{
        let oauthValidator = new OauthValidator('wechat');
        oauthValidator.validate('token');
    ...
    context('login with qq', ()=>{
      beforeEach(()=>{
        getStub =
          sinon.stub(request, 'getAsync')
               .returns(Promise.resolve([{}, {openid: 'yyyy'}]));
      });
